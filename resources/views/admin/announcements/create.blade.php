<x-app-layout>
<div class="admin-shell admin-announce-create-page">
    {{-- HERO --}}
    <div class="relative overflow-hidden border border-[#3A342D]"
         style="
                ...
                <div class="shrink-0">
                    <a href="{{ route('announcements.index') }}" class="admin-hero-action admin-hero-action-secondary"
                       style="
                            ...
                            gap: var(--hs-space-2);
                        "
                       onmouseover="this.style.transform='translateY(-3px)'; this.style.boxShadow='var(--hs-shadow-lg)';"
                       onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='var(--hs-shadow-md)';">
    ...
    <div class="admin-panel-card admin-form-panel">
            <div class="admin-panel-head">
                <div>
                    ...
                    <p class="admin-panel-sub">Write and publish a resident-facing notice.</p>
                </div>
            ...
            <form method="POST"
                  ...
                  data-submitting-text="Publishing Announcement...">
                @csrf
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="8"
                        required
                        class="admin-input admin-textarea">{{ old('content') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="admin-datetime-row">
                        <input type="date" class="admin-input admin-datetime-date" data-dt-date="starts_at" min="{{ now()->toDateString() }}">
                        <select class="admin-input admin-select admin-datetime-time" data-dt-time="starts_at">
                            <option value="">— Time —</option>
                            @for($h = 0; $h < 24; $h++)
                                @foreach(['00','30'] as $m)
                                    @php $val = sprintf('%02d:%s', $h, $m); $label = \Carbon\Carbon::createFromFormat('H:i', $val)->format('g:i A'); @endphp
                                    <option value="{{ $val }}">{{ $label }}</option>
                                @endforeach
                            @endfor
                        </select>
                    </div>
                    <input type="hidden" name="starts_at" value="{{ old('starts_at') }}" data-dt-hidden="starts_at">
                ...
                <div>
                    ...
                    <select name="expiration_option" class="admin-input admin-select" data-expiration-select required>
                        <option value="7_days" @selected(old('expiration_option', '7_days') === '7_days')>7 Days</option>
                        <option value="24_hours" @selected(old('expiration_option') === '24_hours')>24 Hours</option>
                        <option value="30_days" @selected(old('expiration_option') === '30_days')>30 Days</option>
                        <option value="custom" @selected(old('expiration_option') === 'custom')>Custom Date...</option>
                        <option value="never" @selected(old('expiration_option') === 'never')>Never Expire (Pin to Top)</option>
                    </select>
                </div>
                ...
                <div data-custom-expiration style="display: none;">
                    ...
                    <div class="admin-datetime-row">
                        <input type="date" class="admin-input admin-datetime-date" data-dt-date="custom_expires_at" min="{{ now()->addDay()->toDateString() }}">
                    ...
                    <input type="hidden" name="custom_expires_at" value="{{ old('custom_expires_at') }}" data-dt-hidden="custom_expires_at">
                </div>
...
// Combine date + time selects into the hidden datetime-local field
...
.admin-shell.admin-announce-create-page {
...
@media (max-width:560px) {
    .admin-primary-btn, .admin-secondary-btn, .admin-shell.admin-announce-create-page > div:first-of-type a {
        width: 100%;
        justify-content: center;
    }
    .admin-form-actions {
