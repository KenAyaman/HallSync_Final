@if($ticket->status === 'pending_approval')
    <div class="flex gap-2">
        <form method="POST"
              action="{{ route('tickets.approve', $ticket) }}"
              class="inline"
              data-confirm-message="Approve this ticket?"
              data-prevent-double-submit
              data-submitting-text="Approving...">
            @csrf @method('POST')
        ...
        <form method="POST"
              action="{{ route('tickets.reject', $ticket) }}"
              class="inline"
              data-confirm-message="Reject this ticket?"
              data-prevent-double-submit
              data-submitting-text="Rejecting...">
            @csrf @method('POST')
