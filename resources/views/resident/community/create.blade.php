<x-app-layout>
    <div class="community-create-page">
        <div class="community-create-topbar">
            <a href="{{ route('community.index') }}" class="resident-back-link resident-create-back">← Back</a>
        </div>
        ...
        <div class="community-create-grid">
            <section class="community-create-panel">
                ...
                <form method="POST"
                      ...
                      id="postForm"
                      class="community-create-form"
                      x-data='{ body: @js(old("content", "")), bodyCount: {{ strlen(old("content", "")) }}, maxBody: 500 }'
                      data-prevent-double-submit
                      data-submitting-text="Submitting Post...">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="community-category-grid">
                            <label class="community-category-card" data-type="discussion">
                                <input type="radio" name="type" value="discussion" required {{ old('type', 'discussion') === 'discussion' ? 'checked' : '' }}>
                            ...
                            <label class="community-category-card" data-type="lost_found">
                                <input type="radio" name="type" value="lost_found" required {{ old('type') === 'lost_found' ? 'checked' : '' }}>
                            ...
                            <label class="community-category-card" data-type="buy_sell">
                                <input type="radio" name="type" value="buy_sell" required {{ old('type') === 'buy_sell' ? 'checked' : '' }}>
                            ...
                            <label class="community-category-card" data-type="event">
                                <input type="radio" name="type" value="event" required {{ old('type') === 'event' ? 'checked' : '' }}>
                            ...
                            <label class="community-category-card" data-type="other">
                                <input type="radio" name="type" value="other" required {{ old('type') === 'other' ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            x-model="body"
                            @input="bodyCount = $el.value.length"
                            maxlength="500"
                            required
                        ...
                        <div class="community-char-counter"
                             :class="{ 'is-near-limit': bodyCount > 450 }">
                            <span x-text="bodyCount"></span> / 500
                        </div>
                    </div>
    ...
    <script>
        ...
        function updateTitle() {
            ...
            customTitleInput.toggleAttribute('required', selectedType?.value === 'other');
        ...
        document.getElementById('postForm').addEventListener('submit', function (e) {
            ...
            if (!selectedType) {
                e.preventDefault();
                window.appToast?.('error', 'Please select a category.');
                return;
            }
            ...
            if (selectedType.value === 'other') {
                ...
                if (!customTitle) {
                    e.preventDefault();
                    window.appToast?.('error', 'Please enter a title for your post.');
                    return;
                }
            }
            ...
            if (!content) {
                e.preventDefault();
                window.appToast?.('error', 'Please write something before posting.');
                return;
            }
...
.community-create-head h2, .community-create-subhead h3 {
    margin: 0;
    color: #F0E9DF;
    font-size: 1.5rem;
...
.community-create-head p, .community-create-subhead p {
    margin: 4px 0 0;
    color: #8A7A66;
    font-size: 0.95rem;
    line-height: 1.6;
...
.community-create-help, .community-create-footer-note {
    margin: 8px 0 0;
    color: #8A7A66;
    font-size: 0.8rem;
    line-height: 1.7;
...
.community-preview-block img, .community-preview-block video {
    width: 100%;
    max-height: 220px;
    object-fit: cover;
    border-radius: 12px;
...
@media (max-width:768px) {
    ...
    .community-category-grid, .community-preview-area {
