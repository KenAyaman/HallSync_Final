        ...
        <section class="community-edit-panel">
            ...
            <form method="POST" action="{{ route('community.update', $communityPost) }}" enctype="multipart/form-data" id="postForm" class="community-edit-form" data-prevent-double-submit data-submitting-text="Saving Changes...">
                    ...
                    <div class="community-category-grid">
                        @foreach ([
                            ...
                            <label class="community-category-card" data-type="{{ $type }}">
                                <input type="radio" name="type" value="{{ $type }}" required {{ $oldType === $type ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $communityPost->title) }}" class="community-edit-input" placeholder="What is this post about?" required maxlength="255">
                </div>

                <div>
                    <label class="community-edit-label" for="content">What's on your mind?</label>
                    <textarea name="content" id="content" rows="7" class="community-edit-input community-edit-textarea" placeholder="Share your thoughts, updates, or questions with the community..." required>{{ old('content', $communityPost->content) }}</textarea>
                </div>
                    ...
                    @if($communityPost->image_path || $communityPost->video_path)
                        <div class="community-edit-current-media">
                            ...
                            @if($communityPost->image_path)
                                <img src="{{ $communityPost->image_url }}" alt="{{ $communityPost->title }}">
                            @endif
                            @if($communityPost->video_path)
                                <video controls>
                                    <source src="{{ $communityPost->video_url }}">
                                </video>
                            @endif
                        </div>
                    @endif
...
.community-edit-head h1, .community-edit-subhead h2 {
    margin: 0;
    color: #F0E9DF;
    font-size: 1.5rem;
...
.community-edit-head p, .community-edit-subhead p {
    margin: 4px 0 0;
    color: #8A7A66;
    font-size: 0.95rem;
}
...
.community-edit-current-media img, .community-edit-current-media video, .community-preview-block img, .community-preview-block video {
    width: 100%;
    max-height: 320px;
    object-fit: cover;
    border-radius: 16px;
