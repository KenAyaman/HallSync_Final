<x-app-layout>
    <div class="community-show-page">
        <div class="community-show-topbar">
            <a href="{{ route('community.index') }}" class="resident-back-link">Back to Community</a>
        </div>
        ...
        <section class="community-show-panel">
            ...
            @auth
                <form method="POST"
                      ...
                      x-data="{ comment: '', commentCount: 0 }"
                      data-prevent-double-submit
                      data-submitting-text="Posting Comment...">
                    @csrf
                    <div class="community-comment-composer">
                        ...
                        <div class="community-comment-compose-body">
                            ...
                            <textarea name="content"
                                      ...
                                      x-model="comment"
                                      @input="commentCount = $el.value.length"
                                      placeholder="Write a comment..."
                                      required
                                      maxlength="300"></textarea>
                            <div class="community-char-counter"
                                 :class="{ 'is-near-limit': commentCount > 270 }">
                                <span x-text="commentCount"></span> / 300
                            </div>
                        </div>
                    </div>
            ...
            <div class="community-comment-list" data-progressive-list>
                @forelse($post->comments as $comment)
                    <div class="community-comment-item" data-progressive-item x-data="{ editing: false }">
                        <div class="community-comment-avatar">
                            ...
                            @auth
                                @if(auth()->id() === $comment->user_id || auth()->user()->role === 'manager')
                                    ...
                                    <div class="community-comment-actions" x-show="!editing">
                                        ...
                                        <form method="POST"
                                              action="{{ route('community.comments.destroy', $comment) }}"
                ...
                @empty
                    <x-resident-empty-state compact icon="community" title="No comments yet" description="Be the first resident to join the conversation." />
                @endforelse
            </div>
        </section>
    </div>
...
.community-char-counter.is-near-limit {
    color: #bd5349;
    font-weight: 700;
}
.community-show-page {
...
.community-post-card, .community-show-meta-item, .community-comment-item {
...
.community-post-head, .community-comment-item {
    display: flex;
    gap: 14px;
}
.community-post-head {
...
.community-post-avatar, .community-comment-avatar {
    width: 48px;
    height: 48px;
    border-radius: 16px;
    display: flex;
...
.community-post-avatar img, .community-comment-avatar img {
    width: 100%;
    height: 100%;
    object-fit: cover;
}
.community-post-author strong, .community-comment-meta strong {
    display: block;
    color: #F0E9DF;
    font-size: 0.98rem;
}
.community-post-author span, .community-comment-meta span {
    color: #8A7A66;
    font-size: 0.86rem;
}
.community-post-type, .community-post-status {
    padding: 8px 14px;
    border-radius: 999px;
    font-size: 0.76rem;
    font-weight: 700;
...
.community-post-copy p, .community-comment-body p {
    margin: 0;
    color: #B8AB98;
    line-height: 1.8;
    font-size: 0.96rem;
...
.community-post-media img, .community-post-media video {
    width: 100%;
    height: 100%;
    aspect-ratio: 16 / 9;
    object-fit: cover;
...
@media (max-width:560px) {
    ...
    .community-post-badges, .community-comment-meta {
        flex-wrap: wrap;
    }
    .community-thread-summary-grid {
    ...
    .community-post-owner-actions a, .community-post-owner-actions form, .community-post-owner-actions button {
        width: 100%;
    }
    .community-post-owner-actions form {
        display: flex;
