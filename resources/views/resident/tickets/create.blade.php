<x-app-layout>
    <div class="resident-ticket-create-page">
        <div class="resident-ticket-create-topbar">
                ...
                <form method="POST" action="{{ route('tickets.store') }}" enctype="multipart/form-data" id="ticketForm" class="resident-ticket-create-form" data-prevent-double-submit data-submitting-text="Submitting Request...">
                    ...
                    <div>
                        ...
                        <div class="resident-ticket-priority-grid">
                            @foreach ([
                                ...
                                <label class="resident-ticket-priority-card {{ old('priority', 'medium') === $priority ? 'is-active' : '' }}" data-priority="{{ $priority }}">
                    ...
                    <div class="resident-ticket-upload-panel">
                        <div class="resident-ticket-upload-head">
                            <div>
                                <h3>Attachments</h3>
                                <p>Upload a photo or a short video if it helps explain the issue. <span class="dnd-hint">Drag and drop files here if your browser supports it.</span></p>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.querySelectorAll('.resident-ticket-priority-card input[type="radio"]').forEach((radio) => {
...
.resident-ticket-create-hero, .resident-ticket-create-panel {
...
.resident-ticket-create-input, .resident-ticket-create-input-file {
    width: 100%;
    padding: 14px 16px;
    border: 1px solid rgba(214, 168, 91, 0.14);
    border-radius: 16px;
...
.resident-ticket-priority-copy, .resident-ticket-upload-note {
    color: #B8AB98;
    font-size: 0.9rem;
    line-height: 1.7;
}
...
.resident-ticket-upload-grid, .resident-ticket-preview-grid {
    display: grid;
    grid-template-columns: repeat(2, minmax(0, 1fr));
    gap: 14px;
    margin-top: 16px;
...
@media (max-width:768px) {
    ...
    .resident-ticket-priority-grid, .resident-ticket-upload-grid, .resident-ticket-preview-grid {
...
@media (max-width:560px) {
    ...
    .resident-ticket-create-btn, .resident-ticket-create-btn-secondary {
        width: 100%;
    }
    .resident-ticket-create-actions {
        flex-direction: column;
