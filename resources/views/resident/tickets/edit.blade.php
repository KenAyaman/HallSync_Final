<x-app-layout>
    <div class="resident-ticket-edit-page">
        <div class="resident-ticket-edit-topbar">
            ...
            <div class="resident-ticket-edit-actions">
                @if($ticket->status === 'pending_approval')
                    <form method="POST"
                          ...
                          data-confirm-message="Delete this request? This action cannot be undone."
                          data-prevent-double-submit
                          data-submitting-text="Deleting...">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="resident-ticket-edit-btn resident-ticket-edit-btn-danger">Delete Request</button>
                    </form>
                @endif
            </div>
        </section>
            ...
            <section class="resident-ticket-edit-panel resident-ticket-edit-main-panel">
                ...
                <form method="POST" action="{{ route('tickets.update', $ticket) }}" enctype="multipart/form-data" id="ticketForm" class="resident-ticket-edit-form" data-prevent-double-submit data-submitting-text="Saving Changes...">
                    ...
                    <div>
                        ...
                        <div class="resident-ticket-priority-grid">
                            <label class="resident-ticket-priority-card" data-priority="low">
                                <input type="radio" name="priority" value="low" required {{ old('priority', $ticket->priority) === 'low' ? 'checked' : '' }}>
                            ...
                            <label class="resident-ticket-priority-card" data-priority="medium">
                                <input type="radio" name="priority" value="medium" required {{ old('priority', $ticket->priority) === 'medium' ? 'checked' : '' }}>
                            ...
                            <label class="resident-ticket-priority-card" data-priority="critical">
                                <input type="radio" name="priority" value="critical" required {{ old('priority', $ticket->normalized_priority) === 'critical' ? 'checked' : '' }}>
                            ...
                            <div class="resident-ticket-edit-attachment-stack">
                                @if($ticket->image_path)
                                    <div class="resident-ticket-edit-attachment-card">
                                        <img src="{{ $ticket->image_url }}" alt="Current ticket image" class="resident-ticket-edit-attachment-thumb">
                    ...
                    <div class="resident-ticket-edit-form-actions">
                        ...
                        <a href="{{ route('tickets.index') }}" class="resident-ticket-edit-btn resident-ticket-edit-btn-secondary">
                            Cancel
                        </a>
                    </div>
    ...
    <script>
        ...
        attachmentInput.addEventListener('change', function (e) {
            ...
            if (file.type.startsWith('image/') && file.size > 2 * 1024 * 1024) {
                window.appToast?.('error', 'Image must be less than 2MB.');
                attachmentInput.value = '';
                return;
            }

            if (file.type.startsWith('video/') && file.size > 10 * 1024 * 1024) {
                window.appToast?.('error', 'Video must be less than 10MB.');
                attachmentInput.value = '';
                return;
            }
        ...
        document.getElementById('ticketForm').addEventListener('submit', function (e) {
            ...
            if (!description) {
                e.preventDefault();
                window.appToast?.('error', 'Please describe the issue.');
                return;
            }
...
.resident-ticket-edit-topbar {
    display: flex;
    align-items: center;
    justify-content: flex-start;
}
.resident-ticket-edit-hero, .resident-ticket-edit-panel {
...
.resident-ticket-edit-actions form {
    margin: 0;
}
...
.resident-ticket-edit-head h2, .resident-ticket-edit-subhead h3 {
    margin: 0;
    color: #F0E9DF;
    font-size: 1.5rem;
    font-family: 'Playfair Display', serif;
...
.resident-ticket-edit-head p, .resident-ticket-edit-subhead p {
    margin: 4px 0 0;
    color: #8A7A66;
    font-size: 0.95rem;
    line-height: 1.6;
...
.resident-ticket-edit-subpanel, .resident-ticket-edit-meta-item, .resident-ticket-edit-attachment-card {
    background: rgba(255, 255, 255, 0.03);
...
.resident-ticket-edit-attachment-stack, .resident-ticket-edit-inline-meta {
    display: grid;
    gap: 12px;
}
...
.resident-ticket-edit-attachment-copy strong, .resident-ticket-edit-meta-item strong {
    display: block;
    color: #F0E9DF;
    font-size: 0.92rem;
    line-height: 1.6;
...
.resident-ticket-preview-block img, .resident-ticket-preview-block video {
    width: 100%;
    max-height: 220px;
    object-fit: cover;
    border-radius: 12px;
...
@media (max-width:768px) {
    ...
    .resident-ticket-priority-grid, .resident-ticket-preview-area, .resident-ticket-edit-inline-meta {
